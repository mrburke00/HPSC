...
class mpiInfo
{
 public:
  ...
  void ExchangeBoundaryInfo(VD &Solution, VD &b)
  {
	...
	sLOOP phiT[s] = Solution[ pid(  TO-DO in LAB ) ]; 
	sLOOP phiB[s] = Solution[ pid(  TO-DO in LAB ) ]; 
	...
	if ( nei_n >= 0 ) { err = MPI_Irecv( TO-DO in LAB )
	if ( nei_s >= 0 ) { err = MPI_Irecv( TO-DO in LAB )
	if ( nei_e >= 0 ) { err = MPI_Irecv( TO-DO in LAB )
	if ( nei_w >= 0 ) { err = MPI_Irecv( TO-DO in LAB )
	
	// (1.5) If new information was received, store it in the candy-coating values

	if ( nei_n >= 0 ) sLOOP {   TO-DO in LAB ; }
	if ( nei_s >= 0 ) sLOOP {   TO-DO in LAB ; }
	if ( nei_e >= 0 ) tLOOP {   TO-DO in LAB ; }
	if ( nei_w >= 0 ) tLOOP {    TO-DO in LAB ; }
	
	if ( nei_n >= 0 ) sLOOP Solution[ pid( s        , nRealy+1) ] =   TO-DO in LAB ; 
	if ( nei_s >= 0 ) sLOOP Solution[   TO-DO in LAB            ] =   TO-DO in LAB ; 
	if ( nei_e >= 0 ) tLOOP Solution[   TO-DO in LAB            ] =   TO-DO in LAB ; 
	if ( nei_w >= 0 ) tLOOP Solution[   TO-DO in LAB            ] =   TO-DO in LAB ; 
	
	// (1.2) Apply exchanged information as BCs
	
	if ( nei_n >= 0 ) sLOOP b[ pid ( s        , nRealy+1 ) ] =    TO-DO in LAB 
	if ( nei_s >= 0 ) sLOOP b[ TO-DO in LAB                ] =    TO-DO in LAB 
	if ( nei_e >= 0 ) tLOOP b[ TO-DO in LAB                ] =    TO-DO in LAB 
	if ( nei_w >= 0 ) tLOOP b[ TO-DO in LAB                ] =    TO-DO in LAB 
  }
