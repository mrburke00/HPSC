...
int main(  int argc, char *argv[] )
{
  ...
  double **A   = Array2D_double(nField+1,nField+1);
  double *x    = Array1D_double(nField+1);
  double *prod = Array1D_double(nField+1);
  
  double *A_acc    = Array1D_double_acc((nField+1)*(nField+1));
  double *x_acc    = Array1D_double_acc(nField+1);
  double *prod_acc = Array1D_double_acc(nField+1);
...
#pragma acc parallel loop deviceptr(A_acc,x_acc,prod_acc) 
      rLOOP { x_acc[r] = 1.; prod_acc[r] = 1.; }

#pragma acc parallel loop deviceptr(A_acc,x_acc,prod_acc) 
      rLOOP 
        cLOOP 
          A_acc[r*nField + c] = r*10. + c;

  StartTimer(t0);

#pragma acc parallel loop deviceptr(A_acc,x_acc,prod_acc) 
      rLOOP  x_acc[r] = x_acc[r] + prod_acc[r] ; 

#pragma acc parallel loop reduction(+:dot)  deviceptr(A_acc,x_acc,prod_acc) 
      rLOOP dot += x_acc[r] * prod_acc[r] ; 
	...
	rLOOP
	  {
	    prod_acc[r] = 0.;
	    cLOOP
	      prod_acc[r] += A_acc[r*nField+c]*x_acc[c];
	  }

  EndTimer(" Add,Dot,Ax on GPU                 ",t0,t1);
  printf("GPU dot product = %15.5e\n",dot);
