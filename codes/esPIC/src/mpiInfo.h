...
class mpiInfo
{
 public:
  ...
  void ExchangeBoundaryInfo(VD &Solution, VD &b)
  {
	...
	sLOOP phiT[s] = Solution[ pid(    s , nRealy-1 ) ];   // LabReplace("pid(    s , nRealy-1 )" , "      TO-DO in LAB    " )
	sLOOP phiB[s] = Solution[ pid(    s ,    2 ) ];       // LabReplace("pid(    s ,    2 )" , "      TO-DO in LAB    " )
	
	// (1.2) Put them into communication arrays
	
	sLOOP phiSend_n[s] = phiT[s];
	sLOOP phiSend_s[s] = phiB[s];
	tLOOP phiSend_e[t] = phiR[t];        // LabReplace("phiR[t]","     TO-DO in LAB  " )
	tLOOP phiSend_w[t] = phiL[t];        // LabReplace("phiL[t]","     TO-DO in LAB  " )
	...
	if ( nei_n >= 0 ) sLOOP { phiT[s] = phiRecv_n[s] ; }   // LabReplace("phiRecv_n[s]" , "  TO-DO in LAB")
	if ( nei_s >= 0 ) sLOOP { phiB[s] = phiRecv_s[s] ; }   // LabStrip
	if ( nei_e >= 0 ) tLOOP { phiR[t] = phiRecv_e[t] ; }   // LabStrip
	if ( nei_w >= 0 ) tLOOP { phiL[t] = phiRecv_w[t] ; }   // LabStrip
	
	if ( nei_n >= 0 ) sLOOP Solution[ pid( s        , nRealy+1) ] = phiT[s];     // LabReplace("phiT[s];","  TO-DO in LAB  ")
	if ( nei_s >= 0 ) sLOOP Solution[ pid( s        , 0       ) ] = phiB[s];     // LabStrip
	if ( nei_e >= 0 ) tLOOP Solution[ pid( nRealx+1 , t       ) ] = phiR[t];     // LabStrip
	if ( nei_w >= 0 ) tLOOP Solution[ pid( 0        , t       ) ] = phiL[t];     // LabStrip
	
	// (1.2) Apply exchanged information as BCs
	
	if ( nei_n >= 0 ) sLOOP b[ pid ( s        , nRealy+1 ) ] = phiT[s] ;      // LabReplace("phiT[s]" , "     TO-DO in LAB  ")
	if ( nei_s >= 0 ) sLOOP b[ pid ( s        ,    0     ) ] = phiB[s] ;      // LabStrip
	if ( nei_e >= 0 ) tLOOP b[ pid ( nRealx+1 ,    t     ) ] = phiR[t] ;      // LabStrip
	if ( nei_w >= 0 ) tLOOP b[ pid ( 0        ,    t     ) ] = phiL[t] ;      // LabStrip
  }
